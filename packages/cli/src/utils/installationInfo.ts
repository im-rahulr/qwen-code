...
import { isGitRepository } from 'codec-core';
...
export function getInstallationInfo(
  ...
  try {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g codec@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add codec@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g codec@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g codec@latest';
    return {
